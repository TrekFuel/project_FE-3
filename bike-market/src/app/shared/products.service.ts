...
export class ProductsService {
  ...
  getProductById(productId: number): Product {
    return [...this._productsArr].filter((product: Product) => product.id === productId)[0];
  }

  getProductByQuery(productQuery: string): Product[] {
    return [...this._productsArr].filter((product: Product) => product.subcategory === productQuery);
  }
}
